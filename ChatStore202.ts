    ...
    (set, get) => ({
      ...
      sendMessage: async (content) => {
        ...
        const agentName = (() => {
          ...
          if (match?.name) return match.name;
          if (agentId) return agentId;
          // fallback to id if display name not loaded yet
          return 'agent';
        })();
          ...
          const poll = async () => {
            try {
              ...
              if (attempts < maxAttempts) {
                ...
                const chat = chats.find(c => c.id === chatId);
                const lastMsg = chat?.messages[chat.messages.length - 1];
                const timeoutMsg = '❌ Error: Request timed out';
                if (lastMsg?.role === 'assistant') {
                  updateLastMessage(chatId, timeoutMsg);
                } else {
                  addMessage(chatId, { role: 'assistant', content: timeoutMsg, timestamp: new Date().toISOString() });
                }
                set({ loadingChatId: null, isLoading: false, isStreaming: false, currentJob: null });
              }
            } catch (error) {
