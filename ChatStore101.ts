      ...
      sendMessage: async (content) => {
        ...
        try {
          ...
          if (isAgentMode) {
            ...
            // Use recursive setTimeout to ensure streaming continues even when tab is not visible
            let currentContent = '';
            let charIndex = 0;
            ...
            const completeStreaming = () => {
              ...
              // Complete the message instantly
            ...
            const streamNextChar = () => {
              if (charIndex < chars.length) {
                ...
                // Check if tab is visible - if so, complete instantly for better UX
                if (document.visibilityState === 'visible') {
                  // Complete the rest instantly when user returns to tab
                  completeStreaming();
                } else {
                  // Continue streaming in background
                  streamingTimeout = setTimeout(streamNextChar, 1);
                }
              } else {
                set({ loadingChatId: null, isStreaming: false });
              }
            ...
            // Listen for visibility changes to complete streaming instantly when user returns
            const handleVisibilityChange = () => {
              if (document.visibilityState === 'visible' && charIndex < chars.length) {
                completeStreaming();
              }
            };
