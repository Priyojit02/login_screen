      ...
      sendMessage: async (content) => {
        ...
        try {
          ...
          if (isAgentMode) {
            ...
            api.streamJobEvents(
              ...
              (result) => {
                if (result.status === 'completed' && result.result) {
                  ...
                  const finalMessage = result.result.result_message || result.result.text || result.result.message || result.result;
                  updateLastMessage(chatId!, finalMessage);
                  
                  // If DOCX was generated, append download link
                  if (result.result.output_docx_path) {
                    // Use full URL to backend
                    const docxUrl = `${API_BASE_URL}/jobs/${jobId}/docx`;
                  ...
                  updateLastMessage(chatId!, `❌ Error: ${result.error || 'Job failed'}`);
            ...
            // ==========================================
            // NORMAL MODE - Use /chats/{chat_id}/message
            // ==========================================
            const response = await api.sendMessage(chatId, content);
            updateLastMessage(chatId, response.message);
            set({ isLoading: false, isStreaming: false });
          }
          
        } catch (error) {
          console.error('Failed to send message:', error);
          updateLastMessage(chatId!, `❌ Error: ${error instanceof Error ? error.message : 'Failed to send message'}`);
          set({ isLoading: false, isStreaming: false, currentJob: null });
        }
          ...
          const poll = async () => {
            try {
              ...
              if (status.status === 'completed') {
                ...
                updateLastMessage(chatId, `❌ Error: ${status.error || 'Job failed'}`);
                set({ isLoading: false, isStreaming: false, currentJob: null });
                return;
              }
              ...
              if (attempts < maxAttempts) {
                ...
                updateLastMessage(chatId, '❌ Error: Request timed out');
                set({ isLoading: false, isStreaming: false, currentJob: null });
              }
            } catch (error) {
